...
export default function SDLCReview() {
    const { id } = useParams()
    ...
    const [form, setForm] = useState({ decision: 'RECOMMENDED', remarks: '', authorityId: '' })

    useEffect(() => {
        Promise.all([
            api.get(`/claims/${id}`), api.get(`/claims/${id}/decisions`),
            api.get(`/claims/${id}/verifications`), api.get('/authorities?type=SDLC')
        ]).then(([c, d, v, a]) => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            await api.post(`/claims/${id}/decisions`, { decisionLevel: 'SDLC', ...form })
            navigate(`/claims/${id}`)
        } catch (err) {
        ...
        <div>
                ...
                {gsDecision && (
                    <div className="col-md-6">
                        <div className="section-card" style={{ borderLeft: '3px solid #3949ab' }}>
                            ...
                            {[['Decision', gsDecision.decision], ['Resolution No.', gsDecision.resolutionNumber],
                            ['Decided By', gsDecision.decidedBy?.name], ['Remarks', gsDecision.remarks]].map(([l, v]) => (
                                <div className="info-row" key={l}><span className="info-label">{l}</span><span className="info-value">{v || '—'}</span></div>
                            ))}
                        </div>
                    </div>
            ...
            <form onSubmit={handleSubmit}>
                <div className="form-card">
                    <div className="form-section-title">SDLC Review & Recommendation</div>
                    <div className="row g-3">
                        {!user?.authorityId && (
                            <div className="col-12">
                                <label className="form-label">SDLC Authority</label>
                        ...
                        <div className="col-12">
                            <label className="form-label">Recommendation <span className="text-danger">*</span></label>
                        ...
                        <div className="col-12">
                            <label className="form-label">Review Remarks <span className="text-danger">*</span></label>
                            <textarea rows={4} className="form-control form-control-sm" required value={form.remarks}
                                onChange={e => setForm({ ...form, remarks: e.target.value })}
                                placeholder="SDLC review findings, observations, and recommendation rationale..." />
                        </div>
                    </div>
                </div>
                <div className="alert alert-info mt-2 py-2" style={{ fontSize: '0.82rem' }}>
                    If <strong>Recommended</strong>, claim will be forwarded to <strong>DLC</strong> for final decision.
                </div>
                <div className="d-flex gap-2 mt-3">
                    <a href={`/claims/${id}`} className="btn btn-outline-secondary btn-sm">Cancel</a>
                    <button type="submit" className="btn btn-primary btn-sm" disabled={submitting}>
                        {submitting ? <><span className="spinner-border spinner-border-sm me-2" />Submitting...</> : '✓ Submit Review'}
                    </button>
                </div>
            </form>
        </div>
